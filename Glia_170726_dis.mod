...
? Kd ne from Buxton and Brunton, 1986
...
? associated cAMP-dependent glycogen mobilization and ANLS publication. Due to the model's fragility, excessive parameter sensitivity and dt-dependence, we
? have decided not to pursue the molecular modeling strategy of incrementally adding biochemical pathway complexity in a single compartment within the NEURON environment.
...
vL1 			= (kL1*gluco / (kmL1+gluco) )   ? default july2017
...
?cAMPtotal'		= ( .005*(ne/(kDne + ne)) + 200*(1/taucamp * ((hb-ha)/(1+ pow(hc/hk,hd)))) - (2*kgc1*R2C2*pow(cAMPtotal,2)) + (2*k_gc1*R2CcAMP2*CC) - (2*kgc2*R2CcAMP2 * pow(cAMPtotal,2)) + (2*k_gc2 *R2cAMP4*CC) ) + cAMPtotal*(-1/taucamp) ? modified 3/13, removed extra decay term that was in campK because there is a global camp decay term in total equation

cAMPtotal'		= ( .0005*(ne/(kDne + ne)) + .5*(1/taucamp * ((hb-ha)/(1+ pow(hc/hk,hd))) + ha/taucamp ) - (2*kgc1*R2C2*pow(cAMPtotal,2)) + (2*k_gc1*R2CcAMP2*CC) - (2*kgc2*R2CcAMP2 * pow(cAMPtotal,2)) + (2*k_gc2 *R2cAMP4*CC) ) + cAMPtotal*(-1/taucamp) ? combo
